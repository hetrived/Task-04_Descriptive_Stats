...
OUTPUT = Path("output_facebook_ads_pure.txt")
...
def main() -> None:
    ...
    # 2) choose numeric column (prefer 'spend')
    numeric = [c for c, v in cols.items() if v and all(is_num(x) for x in v[:20])]
    target  = "spend" if "spend" in numeric else (numeric[0] if numeric else None)
    ...
    if target:
        ...
        if "page_id" in cols and "ad_id" in cols:
            report += describe_groups(
                "page_id & ad_id", group(data, ("page_id", "ad_id")), target
            )
